...
class ErrorInternalServer extends Error {} // 500
...
class ServerRemoteObject
{
              ...
              promise = this.acl.populateContext(req, aclContext).then(() => {
                ...
                if (Object.keys(responseError).length) {
                  for (var errorName in responseError) {
                    ...
                    var errorValidatePromise = Promise.resolve({isValid: true});
  ...
  buildMethodArgArray(methodArgsConfigArray, requestArgs)
  {
    ...
    methodArgsConfigArray.forEach((argConfig) => {
      ...
      const srcName = argConfig['srcName'] ? argConfig['srcName'] : src;
  ...
  parseOneRequestArg(methodArgConfig, req, res)
  {
    ...
    const aclContext = req ? req['aclContext']: {};
    const aclConditions = req ? req['aclConditions']: {};

    const src = methodArgConfig['src'] ? methodArgConfig['src'] : 'query';
    switch (src) {
      case 'param':
        value = srcName ? params[srcName] : params;
      break;
      case 'query':
        value = srcName ? query[srcName] : query;
      break;
      case 'header':
        value = srcName ? req.get(srcName) : null;
      break;
      case 'body':
        value = srcName ? body[srcName] : body;
      break;
      case 'request':
        value = srcName ? req[srcName] : req;
      break;
      case 'response':
        value = srcName ? res[srcName] : res;
      break;
    }
