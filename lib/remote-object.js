...
class ErrorInternalServer extends ErrorServer {} // 500
class ErrorBadRequest extends ErrorServer {} // 400
class ErrorUnauthorized extends ErrorServer {} // 401
class ErrorForbidden extends ErrorServer {} // 403
class ErrorNotFound extends ErrorServer {} // 404
class ErrorMethodNotAllowed extends ErrorServer {} // 405
class ErrorTooManyRequests extends ErrorServer {} // 429
...
class ServerRemoteObject
{
          ...
          return argValidateSchema.validate(requestArgs).then((validateResult) => {
            if (validateResult.isValid)
            {
              ...
              promise = this.acl.populateContext(req, aclContext).then(() => {
                ...
                requestArgs.aclConditions = isPermitted;
  ...
  requestMin(req)
  {
    ...
    return this.safeToLog(result);
  }
  safeToLog(value, fieldName)
  {
    if (Array.isArray(value)) {
      value.forEach((aValue, aValueX) => {
        value[aValueX] = this.safeToLog(aValue);
      });
    } else if (typeof value == 'object') {
      Object.keys(value).forEeach(fieldName => {
        value[fieldName] = this.safeToLog(value[fieldName], fieldName);
      });
