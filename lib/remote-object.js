      ...
      verbs.forEach(function(verb){
        let middlewareCallback = function(req, res) {
          ...
          methodArgs.forEach(function(methodArgConfig, methodArgIndex){
            ...
            switch (src) {
              case 'params':
                argValidateObject[name] = params;
              break;
              case 'param':
                argValidateObject[name] = params[srcName];
              break;
              case 'query-field':
                argValidateObject[name] = query[srcName];
              break;
              case 'query':
                argValidateObject[name] = query;
              break;
              case 'header-field':
                argValidateObject[name] = req.get(srcName);
              break;
              case 'body-field':
                argValidateObject[name] = body[srcName];
              break;
              case 'body':
                argValidateObject[name] = body;
              break;
              case 'request':
                argValidateObject[name] = req;
              break;
              case 'response':
                argValidateObject[name] = res;
              break;
              case 'remote-object':
                argValidateObject[name] = this;
              break;
              case 'acl-conditions':
                argValidateObject[name] = 'acl-conditions'; // This is just a placeholder which is populated after ACLs have been checked
              break;
            }
            ...
            argumentValues.push(argValidateObject[name]);
            aclContext[name] = argValidateObject[name];
          });
          
          if (validateResult.isValid) 
          {
            if (this.object[method] === undefined) throw new Error('Method "' + method + '" is not defined'); 
            this.acl.populateContext(req, aclContext).then(function(){
              return this.acl.isPermitted(endpointName, aclContext);
            }.bind(this)).then(function(isPermitted){
              if (isPermitted === false) {
                throw new ErrorUnauthorized;
              }
              var aclConditions = isPermitted;
              
              // If ACL conditions have been spceified as a endpoint argument then set them to args array
              var aclConditionsIndex = argumentValues.indexOf('acl-conditions');
              argumentValues[aclConditionsIndex] = aclConditions;
