...
class ServerAcl
{
  ...
  {
    this.roleAssessor['all'] = new RoleAssessorAll;
    this.roleAssessor['authed'] = new RoleAssessorAuthed;
    this.roleAssessor['owner'] = new RoleAssessorOwner;
  }
      ...
      promise = promise.then(function(initResult){
        ...
        return this.hasRole(role, context).then(function(userHasRole){
          ...
          if (typeof userHasRole == 'object') {
            ...
            if (initConditions) {
              // The previous rol assessor returned conditions - these still apply so we return them
              result = initConditions;
            } else {
  ...
  getRules(endpointName)
  {
    ...
    var endpoint = endpointName && this.config['endpoints'][endpointName] ? this.config['endpoints'][endpointName] : {};
