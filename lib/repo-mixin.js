...
class ServerRepoMixin
{
  _find(requestQuery, aclConditions)
  {
    ...
    return this.find(query.where, query.options).then((objects) => {
      return objects;
    });
  }
  _findOne(requestQuery, aclConditions)
  {
    var query = this._parseQuery(clone(requestQuery));
    this._applyAclConditions(query['where'], aclConditions);
    return this.findOne(query.where, query.options).then((object) => {
  ...
  _existsByPkey(pkey, requestQuery, aclConditions)
  {
    ...
    return this.findOne(query.where, fields, query.options).then((objects) => {
  ...
  _upsertOne(requestBody, aclConditions)
  {
    ...
    return this.updateOne(where, {$set: data}, {upsert: true, stripPrivate: true});
  }
  _updateOneByPkey(pkey, requestBody, aclConditions)
  {
    ...
    this._applyAclConditions(where, aclConditions);
    return this.updateOne(where, {$set: data}, {stripPrivate: true});
  }
  _insert(requestBody)
  {
    const data = requestBody;
    var insertMethod = Array.isArray(data) ? 'insertMany' : 'insertOne';
    return this[insertMethod](data, {stripPrivate: true});
  }
  ...
  _parseQuery(query, mode)
  {
    ...
    var queryOptions = {};
    queryOptions['stripPrivate'] = true; // For the API we always want to strip private fields
