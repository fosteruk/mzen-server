...
class ServerRepoMixin
{
  ...
  _findOne(requestQuery)
  {
    var query = this._parseQuery(clone(requestQuery));
    return this.findOne(query.where, query.options);
  }
  _count(requestQuery)
  {
    var query = this._parseQuery(clone(requestQuery));
  ...
  _existsByPkey(pkey, requestQuery)
  {
  ...
  _insert(requestBody)
  {
    ...
    return this.repo[insertMethod](data);
  }
  _updateOne(requestBody)
  {
    var data = clone(requestBody);
    var where = {};
    where[this.config.pkey] = data[this.config.pkey];
  ...
  _deleteOneByPkey(pkey, requestQuery)
  {
    var query = this._parseQuery(clone(requestQuery));
    query.where[this.config.pkey] = pkey;
