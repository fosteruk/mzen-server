...
var ServerRemoteObject = require('mzen-server/lib/remote-object');
      ...
      if (user) {
        return bcrypt.compare(password, user.password).then(function(res) {
          ...
          if (user.accessToken && user.accessToken.expires > new Date) {
            ...
            var keySourceString = "" + user._id + user.password + (new Date()).getTime() + (Math.floor(Math.random() * 100000) + 1);
            var hash = crypto.createHash('sha256').update(keySourceString).digest('hex');
